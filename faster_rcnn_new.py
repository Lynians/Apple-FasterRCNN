...
class AppleDataset(object):
    ...
    def __getitem__(self, idx):
        ...
        targets = []
        d = {}
        d['boxes'] = torch.as_tensor(bbox, device = device)
        d['labels'] = torch.as_tensor(object_name, dtype = torch.int64, device = device)
        targets.append(d)
...
##Another Backbone(Not working for now)##
